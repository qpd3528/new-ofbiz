...
public class BeaconPrintServices {
    private static final String MODULE = BeaconPrintServices.class.getName();
    private static final String RESOURCE = "BeaconUiLabels";

    public static Map<String, Object> printReportPdf(DispatchContext dctx, Map<String, ? extends Object> context) {
        Locale locale = (Locale) context.get("locale");
        String screenLocation = "component://beacon/widget/beacon/BeaconReportScreens.xml";
        String reportScreenName = "BeaconReport";
        ...
        try {
            ...
            String errMsg = UtilProperties.getMessage(RESOURCE, "BeaconGeneralErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()),
                    locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        } catch (IOException e) {
            String errMsg = UtilProperties.getMessage(RESOURCE, "BeaconIOErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()), locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        }
        ...
        // put the beaconId in the screen context, is a parameter coming into the service
        //reportScreenRenderer.getContext().put("beaconId", context.get("beaconId"));
        ...
        try {
            ...
            String errMsg = UtilProperties.getMessage(RESOURCE, "BeaconSAXErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()),
                    locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        } catch (ParserConfigurationException e) {
            String errMsg = UtilProperties.getMessage(RESOURCE, "BeaconParserConfigurationErrorRenderingScreen", UtilMisc.toMap("errorString",
                    e.toString()), locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        }
        ...
        */
        ...
        if (services.length > 0) {
            ...
            try {
                ...
                String errMsg = UtilProperties.getMessage(RESOURCE, "BeaconUnableToPrintPDFFromXSL-FO", UtilMisc.toMap("errorString",
                        pe.toString()), locale);
